	...
	for {
		...
		case entry := <-commands:
			if entry.cmd[0] == "users" {
				...
				entry.cli.ch <- "irc-server > Local time: " + os.Getenv("TZ") + " " + time.Now().Format("15:04")
				...
				if isIn {
					...
					entry.cli.ch <- "irc-server > User not found on list"
				}
				...
				if isAdmin {
					...
					if exists {
						...
						userToKick.ch <- "irc-server > You were kicked from this channel"
						...
						for cli := range clients {
							cli.ch <- "irc-server > [" + userToKick.username + "] was kicked from channel"
						}
				...
				if exists {
					userToMsg.ch <- "whisper: " + entry.cli.username + " > " + strings.Join(entry.cmd[2:], " ")
					entry.cli.ch <- "whisper: " + entry.cli.username + " > " + strings.Join(entry.cmd[2:], " ")
				} else {
					entry.cli.ch <- "irc-server > User not found"
				}
			} else if true {
...
func handleConn(conn net.Conn) {
	...
	ch <- "irc-server > Your user [" + username + "] is successfully logged"
...
func main() {
	if len(os.Args) != 5 {
		fmt.Println("Error: host or port missing")
		return
	}
	serverIP := os.Args[2]
	port := os.Args[4]
